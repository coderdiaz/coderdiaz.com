...
export const Mobile = ({ size = 24 }: Props) => {
  const ref = useRef<any>(null);

  useAnimationFrame((t) => {
    if (ref.current) {
      const y = (1 + Math.sin(t / 500)) * -0.5;
      ref.current.style.transform = `translateY(${y}px)`;
    }
  })

  return (
    <motion.svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path ref={ref} d="M10 19H14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
